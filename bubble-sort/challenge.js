...
//* The default comparator you provide should assume that the two parameters are numbers and that we are sorting the values from smallest to largest. Bubble sort is an O(n^2) algorithm.
...
function bubbleSort(arr, comparatorFn) {
  ...
  for (let i = arr.length; i > 0; i--) {
    ...
    for (let j = 0; j < i - 1; j++) {
      if (comparatorFn(arr[j], arr[j + 1]) > 0) {
        // swap
